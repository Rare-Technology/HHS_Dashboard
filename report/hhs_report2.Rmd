...
knitr::kable(table_summary(selectedData()), 
             align = 'r')
...
```{r Q59, echo=FALSE, message=FALSE, include=TRUE, error = TRUE, warning=FALSE}
   ...
   hhs_Q59 <- selectedData()[,c("ma_name","59_food_procurement")] %>%
         ...
         Q59_summary <- proportion (hhs_Q59$`59_food_procurement`,
                                     hhs_Q59$ma_name,
                                     3,
                                    length(unique(hhs_Q59$`59_food_procurement`)))
 ...
 #Q70
    ...
    Q70_summary <- hhs_Q70 %>%
                       dplyr::group_by(ma_name) %>%
                          summarise (N = n(), 
                                    "Avg Income"  = round(
                                                  mean(as.numeric(as.character(`70_hh_average_income`)), 
    ...
    Q70_lenght <- tapply(as.numeric(as.character(hhs_Q70$`70_hh_average_income`)),
                        hhs_Q70$ma_name,
                          length)
...
```{r Q77, echo= FALSE, message=FALSE,include= TRUE, error = TRUE}
  ...
  kable(Q77_summary, align = 'r') 

```

**Q78**. Proportion of women in community who make financial decisions for the household. 
```{r Q78, echo= FALSE, message=FALSE,include=TRUE, error=TRUE, warning=FALSE}
